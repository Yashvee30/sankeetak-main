...
with mp_hand.Hands(min_detection_confidence=0.5,
    ...
    while True:
        ...
        gesture = "None"
        if len(lmList) != 0:
            # Check for BREATH gesture: All fingers open
            if are_keypoints_visible(lmList, [4, 8, 12, 16, 20]):
                if all(lmList[tipIds[i]][2] < lmList[tipIds[i] - 2][2] for i in range(5)):
                    gesture = "BREATH"

            # Check for CALL gesture: Thumb extended outward, four fingers together and downward
            thumb_extended = lmList[tipIds[0]][1] > lmList[tipIds[0] - 1][1]
            fingers_down_together = all(lmList[tipIds[i]][2] > lmList[tipIds[i] - 2][2] and
                                        abs(lmList[tipIds[i]][1] - lmList[tipIds[i] - 2][1]) < 30 for i in range(1, 5))
            if thumb_extended and fingers_down_together:
                gesture = "CALL"

            # Check for ALLOW gesture: Index finger upward, other fingers downward
            index_finger_up = lmList[tipIds[1]][2] < lmList[tipIds[1] - 2][2]
            other_fingers_down = all(lmList[tipIds[i]][2] > lmList[tipIds[i] - 2][2] for i in range(2, 5))
            ...
            # Check for DELETE gesture: Only pinky finger (keypoint 20) is visible and other fingers and thumb are down
            pinky_up = lmList[tipIds[4]][2] < lmList[tipIds[4] - 2][2]
            others_down = all(lmList[tipIds[i]][2] > lmList[tipIds[i] - 2][2] for i in range(4))
